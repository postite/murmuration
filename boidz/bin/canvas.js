...
boidz_Display.prototype = {
	...
	,render: function() {
		...
		var tmp = this.renderables.keys();
		while(tmp.hasNext()) {
			var renderable = tmp.next();
...
boidz_render_canvas_DebugRender.prototype = {
	...
	,set_scenarioID: function(c) {
		this.scenario.innerText = c == null?"null":"" + c;
		return this.scenarioID = c;
	}
	,clear: function() {
	}
	,beforeEach: function() {
...
boidz_rules_AvoidCollisions.prototype = {
	...
	,modify: function(b) {
		...
		while(_g < _g1.length) {
			...
			dx = b.x - n.x;
			dy = b.y - n.y;
			if(dx * dx + dy * dy > this.squareRadius) {
				continue;
			}
			this.a.x += n.x;
			this.a.y += n.y;
		...
		if(this.proportional) {
			...
			b.d = b.d + boidz_util_Steer.away(b,this.a,this.maxSteer) * (this.get_radius() - dist) / this.get_radius();
		} else {
			b.d = b.d + boidz_util_Steer.away(b,this.a,this.maxSteer);
		}
	}
	,get_radius: function() {
		return this.radius;
...
var boidz_rules_SteerTowardCenter = function(flock,maxSteer) {
	...
	this.flock = flock;
	this.goal = new boidz_rules_SteerTowardGoal(flock.x,flock.y,maxSteer);
};
boidz_rules_SteerTowardCenter.__name__ = ["boidz","rules","SteerTowardCenter"];
boidz_rules_SteerTowardCenter.__interfaces__ = [boidz_IFlockRule];
boidz_rules_SteerTowardCenter.prototype = {
	flock: null
	,goal: null
	,enabled: null
	,before: function() {
		this.goal.x = this.flock.x;
		this.goal.y = this.flock.y;
		return true;
	}
	,modify: function(b) {
		this.goal.modify(b);
	}
	,__class__: boidz_rules_SteerTowardCenter
};
...
boidz_rules_SteerTowardGoal.prototype = {
	...
	,modify: function(b) {
		b.d = b.d + boidz_util_Steer.toward(b,this,this.maxSteer);
	}
...
boidz_rules_SteerTowardZone.prototype = {
	...
	,no: null
	,modify: function(b) {
		if(this.no || this.map.h[b.__id__] == null) {
			return;
		}
		if(this.count > 10) {
			this.doit();
		...
		if(this.go) {
			b.d = b.d + boidz_util_Steer.toward(b,this.map.h[b.__id__],this.maxSteer);
		}
...
boidz_util_Steer.away = function(a,b,max) {
	var d = thx_unit_angle__$Degree_Degree_$Impl_$.normalizeDirection(Math.atan2(a.y - b.y,a.x - b.x) * thx_unit_angle__$Radian_Radian_$Impl_$.ofUnit / thx_unit_angle__$Radian_Radian_$Impl_$.dividerDegree - a.d);
	if(null != max) {
...
boidz_util_Steer.toward = function(a,b,max) {
	var d = thx_unit_angle__$Degree_Degree_$Impl_$.normalizeDirection(Math.atan2(b.y - a.y,b.x - a.x) * thx_unit_angle__$Radian_Radian_$Impl_$.ofUnit / thx_unit_angle__$Radian_Radian_$Impl_$.dividerDegree - a.d);
	if(null != max) {
		var value = Math.abs(d);
		var this1;
		var b1 = max;
...
murmur_Canvas.prototype = {
	...
	,execute: function(dims) {
		...
		debugDisplay.render();
		this.DS.add(function(scenario,val) {
			_gthis.debugRender.set_actionID(val);
			_gthis.debugRender.set_peopleID(_gthis.flock.boids.length);
			_gthis.debugRender.set_scenarioID(scenario);
			debugDisplay.render();
		});
		var this1 = 25;
	...
	,wait: function(state) {
		...
		socket_SocketManager.walkSignal.add(function(dir,sprite) {
			...
			if(state != sprite.state) {
				_gthis.walk.setState(state);
				_gthis.display.addRenderable(_gthis.walk);
				switch(dir) {
				case "left":
					_gthis.walk.back(dir,sprite);
					break;
				case "right":
					_gthis.walk.back(dir,sprite);
					break;
				}
				_gthis.walk.enabled = true;
			}
		});
		socket_SocketManager.emitSignal.add(function(dir1,boid) {
			if(state != boid.state) {
				boid.state = state;
				switch(dir1) {
				case "left":
	...
	,addBoid: function(b) {
		...
		this.debugRender.set_peopleID(this.flock.boids.length);
	}
	...
	,updateVelocity: function() {
		...
		while(_g < _g1.length) {
			var boid = _g1[_g];
			++_g;
			boid.v = this.velocity * (this.randomVelocity?Math.random() * 2:this.velocity);
		}
	}
	,getCanvas: function() {
...
var murmur_DoneSignal = function() {
	msignal_Signal2.call(this,String,String);
};
murmur_DoneSignal.__name__ = ["murmur","DoneSignal"];
murmur_DoneSignal.getInstance = function() {
	if(murmur_DoneSignal.instance != null) {
		return murmur_DoneSignal.instance;
	}
	return murmur_DoneSignal.instance = new murmur_DoneSignal();
};
murmur_DoneSignal.__super__ = msignal_Signal2;
murmur_DoneSignal.prototype = $extend(msignal_Signal2.prototype,{
...
var murmur_Sprite = function(options,state) {
	this.y = 0;
	...
	this.tickCount = 0;
	this.state = state;
	this.ticksPerFrame = Math.floor(60 / options.fps);
	this.numberOfFrames = options.numberOfFrames != 0?options.numberOfFrames:1;
	this.loadImage(options.src);
};
murmur_Sprite.__name__ = ["murmur","Sprite"];
murmur_Sprite.prototype = {
	state: null
	,width: null
	,height: null
	,image: null
	...
	,render: function(context) {
		context.drawImage(this.image,this.currentFrame * this.image.width / this.numberOfFrames,0,this.image.width / this.numberOfFrames,this.height,this.x,this.y,this.image.width / this.numberOfFrames,this.height);
	}
	...
	,back: function() {
		this.x = -this.image.width / this.numberOfFrames;
	}
	,move: function(step) {
		this.x = this.x + step;
	}
...
murmur_Walk.__name__ = ["murmur","Walk"];
murmur_Walk.__interfaces__ = [boidz_IRenderable];
murmur_Walk.prototype = {
	...
	,render: function(render) {
		this.moveX(4);
		this.sprite.update(render.ctx);
		if(this.sprite.x > render.canvas.width) {
			if(this.enabled) {
				murmur_Walk.outBounds.dispatch("right",this.sprite);
			}
		}
	}
...
murmur_scenarios_Scenario.__name__ = ["murmur","scenarios","Scenario"];
murmur_scenarios_Scenario.prototype = {
	hiSpeed: null
	,lowSpeed: null
	,midSpeed: null
	,moreSpeed: null
	...
	,invade: function() {
		...
		murmur_scenarios_Scenario.currentScenario.kill();
		if(this.clientID == 0) {
			this.can.waypoints.set_maxSteer(7);
			this.can.waypoints.radius = 72;
			this.can.waypoints.addGoal(3000,72);
			this.can.waypoints.enabled = true;
			this.dispatch("end invade");
			this.can.randomVelocity = true;
			this.can.velocity = this.veryHiSpeed;
			this.can.updateVelocity();
		}
	}
	,toZone: null
	,towardZone: function(minX,maxX,minY,maxY) {
		var zone = null;
		zone = new boidz_rules_RespectBoundaries(minX,maxX,minY,maxY);
	...
	,towardCenter: function(b) {
		...
		this.dispatch("toward center b" + (b == null?"null":"" + b));
...
murmur_scenarios_TimedScenario.prototype = $extend(murmur_scenarios_Scenario.prototype,{
	...
	,_scene3: function() {
		...
		this.can.updateVelocity();
		this.can.avoidCollisions.enabled = false;
		this.can.flock.addRule(this.can.zone);
		this.dispatch("_scene3/zone/velocity");
	}
	,_scene4: function() {
		this.can.velocity = .6;
		this.can.updateVelocity();
		this.can.zone.enabled = false;
		this.can.avoidCollisions.enabled = false;
		this.can.waypoints.addGoal(Math.random() * this.can.width,Math.random() * this.can.height);
		this.dispatch("_scene4/wayPoint(random)nocollision");
	}
	,_scene5: function() {
		var _gthis = this;
		thx_Timer.delay(function() {
			_gthis.dispatch("end scene5");
			_gthis.can.zone.enabled = false;
			_gthis.can.velocity = _gthis.lowSpeed;
			_gthis.can.updateVelocity();
		},3000);
		...
		this.can.updateVelocity();
		var this1 = 25;
		this.can.zoneBounds = new boidz_render_canvas_ZoneBounds(new boidz_rules_RespectBoundaries(400,800,200,500,50,this1));
		this.can.flock.addRule(this.can.zone);
		this.dispatch("_scene5");
	}
	,kill: function() {
		this.enabled = false;
		this.timer.stop();
...
murmur_scenarios_Slam.prototype = $extend(murmur_scenarios_TimedScenario.prototype,{
	...
	,prepareSplit: function() {
		this.contain();
		this.dispatch("prepareSplit");
	}
	,varie: function() {
		if(this.clientID == 1) {
			this.towardCenter();
		}
		this.dispatch("varie(toCenter");
	}
	,disperse: function() {
		var _gthis = this;
		if(this.clientID == 0) {
			thx_Timer.delay(function() {
				_gthis.dispatch("end disperse");
				_gthis.can.avoidCollisions.enabled = false;
			},3000);
			this.scene1();
			this.can.velocity = this.lowSpeed;
			this.can.updateVelocity();
			this.dispatch("disperse");
		}
	}
	,isole: function() {
		if(this.clientID == 1) {
			this.towardZone(this.can.width / 3 - 200,this.can.width / 3 + 200,this.can.height / 3 - 200,this.can.height / 3 + 200);
		}
	}
	,kill: function() {
		this.can.split.enabled = true;
...
thx_color__$Rgbxa_Rgbxa_$Impl_$.withRed = function(this1,newred) {
	return [newred,thx_color__$Rgbxa_Rgbxa_$Impl_$.get_green(this1),thx_color__$Rgbxa_Rgbxa_$Impl_$.get_blue(this1),this1[3]];
};
thx_color__$Rgbxa_Rgbxa_$Impl_$.withGreen = function(this1,newgreen) {
	return [thx_color__$Rgbxa_Rgbxa_$Impl_$.get_red(this1),newgreen,thx_color__$Rgbxa_Rgbxa_$Impl_$.get_blue(this1),this1[3]];
};
thx_color__$Rgbxa_Rgbxa_$Impl_$.withBlue = function(this1,newblue) {
	return [thx_color__$Rgbxa_Rgbxa_$Impl_$.get_red(this1),thx_color__$Rgbxa_Rgbxa_$Impl_$.get_green(this1),newblue,this1[3]];
};
...
thx_unit_angle__$BinaryDegree_BinaryDegree_$Impl_$.pointToBinaryDegree = function(x,y) {
	return Math.atan2(y,x) * thx_unit_angle__$Radian_Radian_$Impl_$.ofUnit / thx_unit_angle__$Radian_Radian_$Impl_$.dividerBinaryDegree;
};
...
thx_unit_angle__$BinaryDegree_BinaryDegree_$Impl_$.normalize = function(this1) {
	...
	if(n < 0) {
		return thx_unit_angle__$BinaryDegree_BinaryDegree_$Impl_$.turn + n;
	} else {
		return n;
	}
};
thx_unit_angle__$BinaryDegree_BinaryDegree_$Impl_$.normalizeDirection = function(this1) {
	...
	if(normalized >= thx_unit_angle__$BinaryDegree_BinaryDegree_$Impl_$.turn / 2) {
		return normalized - thx_unit_angle__$BinaryDegree_BinaryDegree_$Impl_$.turn;
	} else {
		return normalized;
	}
};
...
thx_unit_angle__$Degree_Degree_$Impl_$.pointToDegree = function(x,y) {
	return Math.atan2(y,x) * thx_unit_angle__$Radian_Radian_$Impl_$.ofUnit / thx_unit_angle__$Radian_Radian_$Impl_$.dividerDegree;
};
...
thx_unit_angle__$Degree_Degree_$Impl_$.normalize = function(this1) {
	...
	if(n < 0) {
		return thx_unit_angle__$Degree_Degree_$Impl_$.turn + n;
	} else {
		return n;
	}
};
thx_unit_angle__$Degree_Degree_$Impl_$.normalizeDirection = function(this1) {
	...
	if(normalized >= thx_unit_angle__$Degree_Degree_$Impl_$.turn / 2) {
		return normalized - thx_unit_angle__$Degree_Degree_$Impl_$.turn;
	} else {
		return normalized;
	}
};
...
thx_unit_angle__$Grad_Grad_$Impl_$.pointToGrad = function(x,y) {
	return Math.atan2(y,x) * thx_unit_angle__$Radian_Radian_$Impl_$.ofUnit / thx_unit_angle__$Radian_Radian_$Impl_$.dividerGrad;
};
...
thx_unit_angle__$Grad_Grad_$Impl_$.normalize = function(this1) {
	...
	if(n < 0) {
		return thx_unit_angle__$Grad_Grad_$Impl_$.turn + n;
	} else {
		return n;
	}
};
thx_unit_angle__$Grad_Grad_$Impl_$.normalizeDirection = function(this1) {
	...
	if(normalized >= thx_unit_angle__$Grad_Grad_$Impl_$.turn / 2) {
		return normalized - thx_unit_angle__$Grad_Grad_$Impl_$.turn;
	} else {
		return normalized;
	}
};
...
thx_unit_angle__$HourAngle_HourAngle_$Impl_$.pointToHourAngle = function(x,y) {
	return Math.atan2(y,x) * thx_unit_angle__$Radian_Radian_$Impl_$.ofUnit / thx_unit_angle__$Radian_Radian_$Impl_$.dividerHourAngle;
};
...
thx_unit_angle__$HourAngle_HourAngle_$Impl_$.normalize = function(this1) {
	...
	if(n < 0) {
		return thx_unit_angle__$HourAngle_HourAngle_$Impl_$.turn + n;
	} else {
		return n;
	}
};
thx_unit_angle__$HourAngle_HourAngle_$Impl_$.normalizeDirection = function(this1) {
	...
	if(normalized >= thx_unit_angle__$HourAngle_HourAngle_$Impl_$.turn / 2) {
		return normalized - thx_unit_angle__$HourAngle_HourAngle_$Impl_$.turn;
	} else {
		return normalized;
	}
};
...
thx_unit_angle__$MinuteOfArc_MinuteOfArc_$Impl_$.pointToMinuteOfArc = function(x,y) {
	return Math.atan2(y,x) * thx_unit_angle__$Radian_Radian_$Impl_$.ofUnit / thx_unit_angle__$Radian_Radian_$Impl_$.dividerMinuteOfArc;
};
...
thx_unit_angle__$MinuteOfArc_MinuteOfArc_$Impl_$.normalize = function(this1) {
	...
	if(n < 0) {
		return thx_unit_angle__$MinuteOfArc_MinuteOfArc_$Impl_$.turn + n;
	} else {
		return n;
	}
};
thx_unit_angle__$MinuteOfArc_MinuteOfArc_$Impl_$.normalizeDirection = function(this1) {
	...
	if(normalized >= thx_unit_angle__$MinuteOfArc_MinuteOfArc_$Impl_$.turn / 2) {
		return normalized - thx_unit_angle__$MinuteOfArc_MinuteOfArc_$Impl_$.turn;
	} else {
		return normalized;
	}
};
...
thx_unit_angle__$Point_Point_$Impl_$.pointToPoint = function(x,y) {
	return Math.atan2(y,x) * thx_unit_angle__$Radian_Radian_$Impl_$.ofUnit / thx_unit_angle__$Radian_Radian_$Impl_$.dividerPoint;
};
...
thx_unit_angle__$Point_Point_$Impl_$.normalize = function(this1) {
	...
	if(n < 0) {
		return thx_unit_angle__$Point_Point_$Impl_$.turn + n;
	} else {
		return n;
	}
};
thx_unit_angle__$Point_Point_$Impl_$.normalizeDirection = function(this1) {
	...
	if(normalized >= thx_unit_angle__$Point_Point_$Impl_$.turn / 2) {
		return normalized - thx_unit_angle__$Point_Point_$Impl_$.turn;
	} else {
		return normalized;
	}
};
...
thx_unit_angle__$Quadrant_Quadrant_$Impl_$.pointToQuadrant = function(x,y) {
	return Math.atan2(y,x) * thx_unit_angle__$Radian_Radian_$Impl_$.ofUnit / thx_unit_angle__$Radian_Radian_$Impl_$.dividerQuadrant;
};
...
thx_unit_angle__$Quadrant_Quadrant_$Impl_$.normalize = function(this1) {
	...
	if(n < 0) {
		return thx_unit_angle__$Quadrant_Quadrant_$Impl_$.turn + n;
	} else {
		return n;
	}
};
thx_unit_angle__$Quadrant_Quadrant_$Impl_$.normalizeDirection = function(this1) {
	...
	if(normalized >= thx_unit_angle__$Quadrant_Quadrant_$Impl_$.turn / 2) {
		return normalized - thx_unit_angle__$Quadrant_Quadrant_$Impl_$.turn;
	} else {
		return normalized;
	}
};
...
thx_unit_angle__$Radian_Radian_$Impl_$.normalize = function(this1) {
	...
	if(n < 0) {
		return thx_unit_angle__$Radian_Radian_$Impl_$.turn + n;
	} else {
		return n;
	}
};
thx_unit_angle__$Radian_Radian_$Impl_$.normalizeDirection = function(this1) {
	...
	if(normalized >= thx_unit_angle__$Radian_Radian_$Impl_$.turn / 2) {
		return normalized - thx_unit_angle__$Radian_Radian_$Impl_$.turn;
	} else {
		return normalized;
	}
};
...
thx_unit_angle__$Revolution_Revolution_$Impl_$.pointToRevolution = function(x,y) {
	return Math.atan2(y,x) * thx_unit_angle__$Radian_Radian_$Impl_$.ofUnit / thx_unit_angle__$Radian_Radian_$Impl_$.dividerRevolution;
};
...
thx_unit_angle__$Revolution_Revolution_$Impl_$.normalize = function(this1) {
	...
	if(n < 0) {
		return thx_unit_angle__$Revolution_Revolution_$Impl_$.turn + n;
	} else {
		return n;
	}
};
thx_unit_angle__$Revolution_Revolution_$Impl_$.normalizeDirection = function(this1) {
	...
	if(normalized >= thx_unit_angle__$Revolution_Revolution_$Impl_$.turn / 2) {
		return normalized - thx_unit_angle__$Revolution_Revolution_$Impl_$.turn;
	} else {
		return normalized;
	}
};
...
thx_unit_angle__$SecondOfArc_SecondOfArc_$Impl_$.pointToSecondOfArc = function(x,y) {
	return Math.atan2(y,x) * thx_unit_angle__$Radian_Radian_$Impl_$.ofUnit / thx_unit_angle__$Radian_Radian_$Impl_$.dividerSecondOfArc;
};
...
thx_unit_angle__$SecondOfArc_SecondOfArc_$Impl_$.normalize = function(this1) {
	...
	if(n < 0) {
		return thx_unit_angle__$SecondOfArc_SecondOfArc_$Impl_$.turn + n;
	} else {
		return n;
	}
};
thx_unit_angle__$SecondOfArc_SecondOfArc_$Impl_$.normalizeDirection = function(this1) {
	...
	if(normalized >= thx_unit_angle__$SecondOfArc_SecondOfArc_$Impl_$.turn / 2) {
		return normalized - thx_unit_angle__$SecondOfArc_SecondOfArc_$Impl_$.turn;
	} else {
		return normalized;
	}
};
...
thx_unit_angle__$Sextant_Sextant_$Impl_$.pointToSextant = function(x,y) {
	return Math.atan2(y,x) * thx_unit_angle__$Radian_Radian_$Impl_$.ofUnit / thx_unit_angle__$Radian_Radian_$Impl_$.dividerSextant;
};
...
thx_unit_angle__$Sextant_Sextant_$Impl_$.normalize = function(this1) {
	...
	if(n < 0) {
		return thx_unit_angle__$Sextant_Sextant_$Impl_$.turn + n;
	} else {
		return n;
	}
};
thx_unit_angle__$Sextant_Sextant_$Impl_$.normalizeDirection = function(this1) {
	...
	if(normalized >= thx_unit_angle__$Sextant_Sextant_$Impl_$.turn / 2) {
		return normalized - thx_unit_angle__$Sextant_Sextant_$Impl_$.turn;
	} else {
		return normalized;
	}
};
...
thx_unit_angle__$Turn_Turn_$Impl_$.pointToTurn = function(x,y) {
	return Math.atan2(y,x) * thx_unit_angle__$Radian_Radian_$Impl_$.ofUnit / thx_unit_angle__$Radian_Radian_$Impl_$.dividerTurn;
};
...
thx_unit_angle__$Turn_Turn_$Impl_$.normalize = function(this1) {
	...
	if(n < 0) {
		return thx_unit_angle__$Turn_Turn_$Impl_$.turn + n;
	} else {
		return n;
	}
};
thx_unit_angle__$Turn_Turn_$Impl_$.normalizeDirection = function(this1) {
	...
	if(normalized >= thx_unit_angle__$Turn_Turn_$Impl_$.turn / 2) {
		return normalized - thx_unit_angle__$Turn_Turn_$Impl_$.turn;
	} else {
		return normalized;
	}
};
